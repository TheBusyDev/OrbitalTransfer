% Create a video for the Alternative Transfer 1
% WARNING:  this script needs 'AlternativeTransfer1.m' to be already run,
%           so that all the necessary variables are initialized.

%% Define Transfer Data
% Orbital elements
a =     [a_i,   a_t,    a_f,    a_f,    a_f];
e =     [e_i,   e_t,    e_f,    e_f,    e_f];
i =     [i_i,   i_i,    i_i,    i_f,    i_f];
OM =    [OM_i,  OM_i,   OM_i,   OM_f,   OM_f];
om =    [om_i,  om_t,   om_i,   om_2,   om_f];
...
% color of each orbit
color = [color_i, color_4, color_2, color_3, color_f];
...
theta_point(2).theta(1).f = [0, pi];
theta_point(2).label(1).f = ["pericenter initial orbit", "apocenter orbit 2"];
theta_point(2).color(1).f = [color_c, color_w];

theta_point(3).theta(2).f = pi;
theta_point(3).label(2).f = "apocenter orbit 2";
theta_point(3).color(2).f = color(3);

theta_point(4).theta(1).f = [theta_2a, theta_2b];
theta_point(4).label(1).f = ["point A", "point B"];
theta_point(4).color(1).f = [color_w, color_c];

theta_point(4).theta(2).f = theta_2;
theta_point(4).label(2).f = "theta 2";
theta_point(4).color(2).f = color(4);

theta_point(5).theta(1).f = [theta_3c_f, theta_3d_f, theta_f];
theta_point(5).label(1).f = ["point C", "point D", "final point"];
theta_point(5).color(1).f = [color_c, color_w, color(5)];

% label to display in legend
orbit_label = ["Initial Orbit", "Bitangent Transfer Orbit", "Orbit 2",...
                "Orbit 3", "Final Orbit"];
...
az = 69.155;            % azimuth angle
el = 27.419;            % elevation angle
...
% plot third orbit
orbit = plotOrbit(a(j+2), e(j+2), i(j+2), OM(j+2), om(j+2), 0, 2*pi, dth, mu, orbit_label(j+2));
